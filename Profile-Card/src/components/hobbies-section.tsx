  ...
  return (
    <div
      className={`flex flex-col gap-[1rem] ${
        displayHobbies ? "my-[1rem]" : "my-[2rem]"
      }`}
    >
      {/* Following is the toggle button */}
      <div className="flex justify-around items-center text-[#3E7B27] text-[1.1rem]">
        <h2
          className={
            !displayHobbies
        ...
        <div className="pt-[0.5rem]">
          <label className="relative inline-flex items-center cursor-pointer">
            ...
            />
            <div className="group peer bg-white rounded-full duration-300 w-16 h-8 ring-2 ring-[#3E7B27] after:duration-300 after:bg-green-500 peer-checked:after:bg-green-500 peer-checked:ring-green-500 after:rounded-full after:absolute after:h-6 after:w-6 after:top-1 after:left-1 after:flex after:justify-center after:items-center peer-checked:after:translate-x-8 peer-hover:after:scale-95"></div>
          </label>
        </div>
        <h2
          className={
      ...
      <div className={!displayHobbies ? "hidden" : "flex justify-around"}>
        {hoddies.map((item, index) => {
          return (
            <p
              key={index}
              className="w-[7rem] h-max text-center rounded-l-md rounded-r-md bg-gradient-to-r from-[#73735A] to-[#445A4A] text-[#fff] font-semibold py-[0.5rem]"
            >
              {item}
            </p>
          );
